...
All of the deployed models showed weak performance in predicting depression score. Specifically, all 5 models of Linear Regression using ordinary least squares regression and L1 regularization (LASSO), neural network, gradient Boosting Regression, and gradient boosting classification performed worse on average than guessing the mean depression score for all participants. Specifically, OLS and LASSO output similar RMSE and R² scores of 4.39 and -0.034, respectively, which were the lowest RMSE scores in all deployed models. Meanwhile, the neural network model had an RMSE of 4.6 and R² of -0.15, which were the highest (least negative) RMSE score and the lowest R² value respectively. The gradient boosting regression model had an RMSE of 4.48 and R² of -0.022, which was the lowest RMSE score. The gradient boosting classification model had an accuracy rate of 60%, a recall rate of 85%, and a precision rate of 64%.
...
![Gradient Boosting: Classification Model with SHAP values](images/classification_model.png){width="75%"}
