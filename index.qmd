...
![Visualization of the first tree in the Gradient Boosting Classifier model](images/tree.png){width="100%"}

# Results

### Performance of the Models

All of the deployed models performed inefficiently at predicting depression score. Specifically, all 5 models of Linear Regression using Ordinary Least Square Error and L1 Error (LASSO), Neural Network, Gradient Boosting Regression, and Gradient Boosting Classification performed worse on average than guessing the mean depression score for all participants. Specifically, OLS and LASSO output similar RMSE and R² scores of 4.39 and -0.034, respectively, which were the lowest RMSE score in all deployed models. Meanwhile, the neural network model had an RMSE of 4.6 and R² of -0.15, which were the highest RMSE score and the lowest R² value. The gradient boosting regression model had an RMSE of 4.48 and R² of -0.022, which was the lowest RMSE score. The gradient boosting classification model had an accuracy rate of 60%, a recall rate of 85%, and a precision rate of 64%.
...
It is noticeable that using all of our variables instead of the subset sized 5 delivered in the Gradient Boosting model output a slightly higher R² score, which suggests that there are likely other variables that have a stronger relationship with depression score that we did not include in our other 5-subset models. Another interesting observation is of the high recall rate but low precision and low accuracy in the SHAP analysis of the Gradient Boosting model, which suggests that while Gradient Boosting accurately predicts a good portion of true positive, it also has a high rate of false positives. This may indicate that there are many predictors that are not actually relevant to depression score. Additionally, the low accuracy (60%) suggests that there may be a lot of noise in the data, and that our predictor variables may not be able to capture the complexity of depression score, which is consistent with our other models performing worse than guessing the mean depression score.
